        ...
        SendContext context, CancellationToken ct = default)
    {
        ...
        var client = services.GetKeyedService<IIntegrationClient>(systemCode)
            ?? throw new IntegrationMessagingException(
                ...
                $"Implement IIntegrationClient and register it with key '{systemCode}'.");

        logger.LogDebug(
            "Dispatching {MessageTypeName} via {SystemCode} | EntityId={EntityId}",
            messageTypeName, systemCode, context.QueueMessage.EntityId);
